...
- **모든 단계를 자세히 설명한다.** 개념의 배경, 이게 뭔지, 왜 적용해야 하는지, 어떻게 쓰는지,
  동작 원리를 짚어준다. 명령어만 던지지 않는다. 코드는 줄별로 설명할 필요 없고, 이해에 필요한
  중요 포인트 위주로 자세히 설명한다. 주석도 곁들인다.
...
**매 덩어리마다 `README.md`를 갱신한다.** README는 프로젝트를 처음 보는 사람이 한눈에
파악하도록: 목표, 무엇을 하는 프로젝트인지, 아키텍처(mermaid), 주요 flow(mermaid),
현재 진행 상태. 개념이 늘어날 때마다 여기 반영해서 사용자가 큰 그림을 잃지 않게 한다.
